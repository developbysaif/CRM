...
export default function MeetingsPage() {
  ...
  return (
    <AppLayout title="Calendar & Meetings" subtitle="Discovery calls, client consultations & scheduling sync">
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', gap: 16 }}>
          <div className="stat-card" style={{ padding: '12px 20px', minWidth: 130, textAlign: 'center' }}>
            <div style={{ fontSize: 24, fontWeight: 800, color: '#0052ff' }}>{upcoming.length}</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Upcoming Calls</div>
          </div>
          <div className="stat-card" style={{ padding: '12px 20px', minWidth: 130, textAlign: 'center' }}>
      ...
      {showForm && (
        <div className="card" style={{ padding: 24, marginBottom: 24 }}>
          <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16 }}>📅 Schedule New Client Consultation</h3>
          <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
              ...
              <input
                ...
                placeholder="Discovery Call with Engineering Lead"
              />
            </div>
            <div className="form-group">
              ...
              >
                {['Discovery Call', 'Demo', 'Proposal Review', 'Negotiation', 'Follow-up', 'Architecture Sync'].map((t) => (
            ...
            <div className="form-group">
              <label className="input-label">Meeting Video Link</label>
              <input
                className="input"
            ...
            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
              <label className="input-label">Meeting Agenda / Scope</label>
              <textarea
                className="input"
                value={form.notes}
                onChange={(e) => setForm((p) => ({ ...p, notes: e.target.value }))}
                placeholder="Discuss project requirements, tech stack, and deliverable milestones..."
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Schedule a discovery call with your clients or let the AI Consultant auto-book time slots."
          action={
          ...
          {upcoming.length > 0 && (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {upcoming.map((m) => (
                  <MeetingCard key={m._id} meeting={m} statusColors={statusColors} />
                ))}
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {past.slice(0, 5).map((m) => (
                  <MeetingCard key={m._id} meeting={m} statusColors={statusColors} past />
                ))}
              </div>
            </div>
          )}
    ...
    <div className="card" style={{ padding: 20, display: 'flex', alignItems: 'flex-start', gap: 16, opacity: past ? 0.7 : 1 }}>
      <div
        style={{
          ...
          height: 48,
          borderRadius: 12,
          background: past ? 'var(--bg-elevated)' : 'var(--gradient-primary)',
          display: 'flex',
      ...
      >
        ...
        <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.85)', fontWeight: 600, marginTop: 2 }}>
          {new Date(m.startTime).toLocaleString('default', { month: 'short' }).toUpperCase()}
        </div>
      </div>
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              ...
              {m.leadId && <span> · Client: {m.leadId.name || 'Prospect'}</span>}
            </div>
          </div>
          <span
            style={{
              ...
              background: `${statusColors[m.status] || '#0052ff'}20`,
              color: statusColors[m.status] || '#0052ff',
            }}
          >
            {m.status}
          </span>
