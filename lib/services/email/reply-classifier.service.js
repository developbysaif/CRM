...
export async function classifyInboundReply(replyText, leadContext = {}) {
  ...
  // Answer is NO / Negative response: strictly HALTS all follow-up messages
  if (
    /^(no|nope|nah|no thanks?|no thank you|not interested)[\.\!\?]?$/i.test(lower) ||
    /\b(not interested|no thanks?|no thank you|pass on this|not looking|don't contact|dont contact|not for us|not now|never|dont need|don't need|no need|no budget)\b/i.test(lower)
  ) {
    return {
      intent: 'Not Interested',
      confidence: 0.98,
      reasoning: 'Client answered NO / declined offer. All future follow-ups permanently terminated (0 tokens burned)',
